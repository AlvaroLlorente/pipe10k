...
_loop_km_begin
c3%Im(:,nh) = (-vel_uz%Im(:,nh)*ad_m1r1(:,m))*mes_D%r(n_,-1)
c3%Re(:,nh) =  (vel_uz%Re(:,nh)*ad_m1r1(:,m))*mes_D%r(n_,-1)
...
ttDT3sq(n_,csta)=uuDT3sq(n_,csta)+sum(p4%Re(:,:,n)**2)
ttDT3(n_,csta)=uuDT3(n_,csta)+sum(p4%Re(:,:,n))
enddo
